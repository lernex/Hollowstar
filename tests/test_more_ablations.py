...
def test_more_core_uses_the_measured_portage_lane():
    ...
    assert primary.measured_tokens_per_second == 576_904
...
def test_wave_one_launchers_keep_the_requested_seed(tmp_path: Path):
    ...
    assert not (tmp_path / "wave1" / "launch-wave-1c.sh").exists()
    assert not (tmp_path / "wave1" / "launch-wave-1d.sh").exists()
    assert "440 APUs" in batch_a
    assert "360 APUs" in batch_b
    for batch in (batch_a, batch_b):
        assert 'METIS_ABLATION_EXCLUDE_NODES:-parrypeak[007,012,020,026,056,063-064]' in batch
...
def test_wave_one_plan_uses_the_measured_two_batch_schedule():
    ...
    assert payload["allocation"]["spare_apus"] == 72
