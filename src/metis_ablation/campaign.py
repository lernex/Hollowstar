...
(
  ...
  while kill -0 "$step_pid" 2>/dev/null; do
    ...
    if [ "$current_size" -ne "$last_size" ]; then
      ...
      if ! timeout --kill-after=2s 15s python3 "{repo_root}/src/metis_ablation/runtime_diagnostics.py" \\
        --run-root "$METIS_ABLATION_OUTPUT/{row}" --job-id "$SLURM_JOB_ID" \\
        --scheduler-timeout 3 >&2; then
        echo "metis: failure snapshot unavailable; preserving the original stalled-step failure" >&2
      fi
