...
def _generated_file_probability(text: str, path: str | None) -> tuple[float, dict[str, Any]] | None:
    if not path:
        return None
    candidate = f"{path}\n{text[:8_192]}".lower()
    markers = [
    ...
    return (1.0 if markers else 0.05), {"path": path, "markers": markers}
...
def derive_normalization_evidence(
    ...
    if profile_name in {"repository_code_v1", "fresh_repository_code_v1"}:
        ...
        if generated_probability is None:
            path_value, path_path = _find(searchable, "path", "repo_path", "file_info.path")
            computed = _generated_file_probability(text, str(path_value) if path_value else None)
            if computed:
                generated_probability, details = computed
