...
def setup_runtime(requested: str | None) -> tuple[torch.device, bool, int, int, int]:
    ...
    if distributed:
        if not torch.cuda.is_available():
            raise RuntimeError("Distributed Metis SFT requires CUDA devices.")
...
def aggregate_interval_stats(
    *,
    ...
    if not distributed:
        return loss_total, updates, micro_batches, tokens
    stats = torch.tensor(
        [loss_total, float(updates), float(micro_batches), float(tokens)],
        device=device,
        dtype=torch.float64,
    )
    dist.all_reduce(stats, op=dist.ReduceOp.SUM)
    return float(stats[0].item()), int(stats[1].item()), int(stats[2].item()), int(stats[3].item())


def broadcast_scalar(value: float, *, device: torch.device, distributed: bool) -> float:
    if not distributed:
        return value
    payload = torch.tensor([value], device=device, dtype=torch.float64)
    dist.broadcast(payload, src=0)
    return float(payload[0].item())
...
def resolve_precision(args: argparse.Namespace, device: torch.device) -> tuple[torch.dtype, bool, object | None, str]:
    ...
    if args.nvfp4:
        if device.type != "cuda":
            raise RuntimeError("NVFP4 SFT requires CUDA.")
    ...
    if fp8_enabled:
        if device.type != "cuda":
            raise RuntimeError("FP8 SFT requires CUDA.")
...
@torch.no_grad()
def evaluate(model, loader: DataLoader, device: torch.device, *, compiled: bool) -> float:
    model.eval()
    losses = []
    for xb, yb, attention_mask in loader:
        ...
        output = model(xb, labels=yb, attention_mask=attention_mask, is_first_microbatch=True)
        tracked_loss = output.lm_loss if output.lm_loss is not None else output.loss
        losses.append(float(tracked_loss.item()))
    model.train()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Supervised fine-tuning for Metis-1.5 MoR chat or reasoning data.")
    ...
    try:
        ...
        train_dataset = train_dataset.map(
            ...
            desc="Tokenizing Metis-1.5 train SFT",
        ).filter(lambda row: len(row["input_ids"]) > 0)
        val_dataset = val_dataset.map(
            lambda row: tokenize_example(row, tokenizer=tokenizer, max_length=args.max_length),
            remove_columns=val_dataset.column_names,
            desc="Tokenizing Metis-1.5 val SFT",
        ).filter(lambda row: len(row["input_ids"]) > 0)
        ...
        while global_step < total_updates:
            ...
            for step_in_epoch, (xb, yb, attention_mask) in enumerate(train_loader, start=1):
                non_blocking = device.type == "cuda"
                xb = xb.to(device, non_blocking=non_blocking)
                yb = yb.to(device, non_blocking=non_blocking)
                attention_mask = attention_mask.to(device, non_blocking=non_blocking)
                ...
                tracked_loss = output.lm_loss if output.lm_loss is not None else output.loss
                interval_loss += float(tracked_loss.item())
                interval_micro_batches += 1
                if output.route_aux_loss is not None:
                    interval_route_aux += float(output.route_aux_loss.item())
                if output.moe_aux_loss is not None:
                    interval_moe_aux += float(output.moe_aux_loss.item())
                if output.mean_depth is not None:
                    interval_mean_depth += float(output.mean_depth.item())
                ...
                if should_step:
                    ...
                    if args.log_interval > 0 and global_step % args.log_interval == 0:
                        ...
                        total_loss, total_updates_logged, total_micro_batches, total_tokens = aggregate_interval_stats(
                        ...
                        if is_main_process(rank):
                            ...
                            print(
                                ...
                                f"depth {interval_mean_depth / max(interval_micro_batches, 1):.2f} | "
                                f"route_aux {interval_route_aux / max(interval_micro_batches, 1):.4f} | "
                                f"moe_aux {interval_moe_aux / max(interval_micro_batches, 1):.4f} | "
                    ...
                    if should_eval:
                        if is_main_process(rank):
                            val_loss = evaluate(model, val_loader, device, compiled=compile_enabled)
                        else:
                            val_loss = 0.0
                        val_loss = broadcast_scalar(val_loss, device=device, distributed=distributed)
                        if is_main_process(rank):
                            print(
                                f"epoch {epoch:3d} | step {global_step:5d} | "
