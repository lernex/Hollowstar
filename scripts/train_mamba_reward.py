...
from metis_mamba import MetisMambaConfig, build_reward_model, cosine_lr, parse_torch_dtype
...
def setup_runtime(requested: str | None) -> tuple[torch.device, bool, int, int, int]:
    ...
    if distributed:
        if not torch.cuda.is_available():
            raise RuntimeError("Distributed Metis reward-model training requires CUDA devices.")
...
def resolve_precision(args: argparse.Namespace, device: torch.device) -> tuple[torch.dtype, bool, object | None, str]:
    ...
    if args.nvfp4:
        if device.type != "cuda":
            raise RuntimeError("NVFP4 reward-model training requires CUDA.")
    ...
    if fp8_enabled:
        if device.type != "cuda":
            raise RuntimeError("FP8 reward-model training requires CUDA.")
...
def build_preference_sequence(prompt: str, response: str) -> str:
    prompt = prompt.strip()
    if prompt.endswith("Assistant:"):
        return f"{prompt} {response.strip()}".strip()
    return f"{prompt}\nAssistant: {response.strip()}".strip()
...
def tokenize_text(
    *,
    ...
    sequence = build_preference_sequence(prompt, response)
    token_ids = tokenizer.encode(sequence, add_special_tokens=False).ids
    bos_id = tokenizer.token_to_id("<bos>")
    eos_id = tokenizer.token_to_id("<eos>")
    output: list[int] = []
    if bos_id is not None:
        output.append(bos_id)
...
def tokenize_pair_example(example: dict, *, tokenizer: Tokenizer, max_length: int) -> dict[str, Any]:
    chosen_ids = tokenize_text(
        prompt=str(example["prompt"]),
        response=str(example["chosen"]),
        tokenizer=tokenizer,
        max_length=max_length,
    )
    rejected_ids = tokenize_text(
        prompt=str(example["prompt"]),
...
def collate_sequences(
    ...
    *,
    ...
    for item in sequences:
        ...
        input_ids.append(item + ([pad_token_id] * pad_len))
        attention_masks.append(([1] * len(item)) + ([0] * pad_len))
    return torch.tensor(input_ids, dtype=torch.long), torch.tensor(attention_masks, dtype=torch.bool)
...
def collate_pair_batch(
    ...
    *,
    ...
    chosen_ids, chosen_mask = collate_sequences(
        [item["chosen_input_ids"] for item in batch],
        pad_token_id,
        pad_to_multiple=pad_to_multiple,
    )
    rejected_ids, rejected_mask = collate_sequences(
    ...
    return chosen_ids, chosen_mask, rejected_ids, rejected_mask, weights


@torch.no_grad()
def evaluate(model, loader: DataLoader, device: torch.device, *, compiled: bool) -> tuple[float, float]:
    model.eval()
    losses = []
    accuracies = []
    for chosen_ids, chosen_mask, rejected_ids, rejected_mask, weights in loader:
        ...
        chosen_output = model(chosen_ids, attention_mask=chosen_mask, is_first_microbatch=True)
        rejected_output = model(rejected_ids, attention_mask=rejected_mask, is_first_microbatch=True)
        reward_margin = chosen_output.rewards - rejected_output.rewards
        pair_loss = (-F.logsigmoid(reward_margin) * weights).mean()
        losses.append(float(pair_loss.item()))
        accuracies.append(float((reward_margin > 0).float().mean().item()))
    model.train()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the Metis-1.5 reward model on pairwise preference data.")
    ...
    try:
        ...
        model.load_state_dict(
            filter_state_dict_for_model(model, checkpoint["model_state_dict"]),
            strict=False,
        )
        ...
        train_dataset = train_dataset.map(
            ...
            desc="Tokenizing Metis-1.5 reward train",
        ).filter(lambda row: len(row["chosen_input_ids"]) > 0 and len(row["rejected_input_ids"]) > 0)
        val_dataset = val_dataset.map(
            lambda row: tokenize_pair_example(row, tokenizer=tokenizer, max_length=args.max_length),
            remove_columns=val_dataset.column_names,
            desc="Tokenizing Metis-1.5 reward val",
        ).filter(lambda row: len(row["chosen_input_ids"]) > 0 and len(row["rejected_input_ids"]) > 0)

        if len(train_dataset) == 0 or len(val_dataset) == 0:
            raise ValueError("Reward-model tokenization produced an empty train or val dataset.")
        ...
        optimizer, optimizer_summary = build_optimizer_from_args(model, args)
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer,
        ...
        if args.resume and latest_checkpoint_path.exists():
            ...
            model.load_state_dict(
                filter_state_dict_for_model(model, resume_checkpoint["model_state_dict"]),
                strict=False,
            )
            ...
            model.to(device=device, dtype=model_dtype)
            if is_main_process(rank):
                print(f"Resuming reward-model training from {latest_checkpoint_path} after epoch {start_epoch - 1}")
        barrier(distributed)
        ...
        while global_step < total_updates:
            ...
            for step_in_epoch, batch in enumerate(train_loader, start=1):
                chosen_ids, chosen_mask, rejected_ids, rejected_mask, pair_weights = batch
                non_blocking = device.type == "cuda"
                chosen_ids = chosen_ids.to(device, non_blocking=non_blocking)
                chosen_mask = chosen_mask.to(device, non_blocking=non_blocking)
                rejected_ids = rejected_ids.to(device, non_blocking=non_blocking)
                ...
                with sync_context:
                    ...
                    chosen_output = train_model(
                        chosen_ids,
                        attention_mask=chosen_mask,
                        is_first_microbatch=((step_in_epoch - 1) % args.grad_accum_steps == 0),
                    )
                    rejected_output = train_model(
                        rejected_ids,
                        attention_mask=rejected_mask,
                        is_first_microbatch=False,
                    )
                    reward_margin = chosen_output.rewards - rejected_output.rewards
                    pair_loss = (-F.logsigmoid(reward_margin) * pair_weights).mean()
                    aux_loss = pair_loss.new_zeros(())
                    if chosen_output.route_aux_loss is not None and rejected_output.route_aux_loss is not None:
                        aux_loss = aux_loss + (
                            0.5 * (chosen_output.route_aux_loss + rejected_output.route_aux_loss)
                            * config.mor_router_aux_loss_coef
                        )
                    if chosen_output.moe_aux_loss is not None and rejected_output.moe_aux_loss is not None:
                        aux_loss = aux_loss + (
                            0.5 * (chosen_output.moe_aux_loss + rejected_output.moe_aux_loss)
                            * config.moe_aux_loss_coef
                        )
                    loss = pair_loss + aux_loss
                    (loss / args.grad_accum_steps).backward()

                interval_loss += float(pair_loss.item())
                interval_accuracy += float((reward_margin > 0).float().mean().item())
                ...
                if should_step:
                    ...
                    if args.log_interval > 0 and global_step % args.log_interval == 0:
                        ...
                        if is_main_process(rank):
                            ...
                            print(
                                ...
                                f"train {total_loss / max(total_micro_batches, 1):.4f} | "
                    ...
                    if should_eval:
                        if is_main_process(rank):
                            val_loss, val_accuracy = evaluate(model, val_loader, device, compiled=compile_enabled)
                        else:
