...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the derived Metis-1.3 stage plan from prepared data.")
    parser.add_argument("--manifest", default="configs/metis13_manifest.json")
    parser.add_argument("--pretrain-meta", default="data/metis13_base/meta.json")
    parser.add_argument("--chat-meta", default="data/metis13_chat_sft/meta.json")
    parser.add_argument("--reasoning-meta", default="data/metis13_reasoning_sft/meta.json")
    parser.add_argument("--output-path", default="artifacts/metis13_plan.json")
    ...
    global_batch_size = local_batch_size * grad_accum_steps
    ...
    chat_plan = build_stage_plan(
        ...
        global_batch_size=int(manifest["chat_sft"]["local_batch_size"]) * int(manifest["chat_sft"]["grad_accum_steps"]),
    ...
    reasoning_plan = build_stage_plan(
        ...
        global_batch_size=int(manifest["reasoning_sft"]["local_batch_size"]) * int(manifest["reasoning_sft"]["grad_accum_steps"]),
