...
ATTENTION_KERNEL_CHOICES = ("eager", "eager_gqa", "sdpa", "nki_flash_2k", "nki_flash_1k")
NKI_FLASH_LSE_DTYPE_CHOICES = ("auto", "float32", "bfloat16")
...
def setup_runtime(device_arg: str) -> Runtime:
    ...
    if distributed:
        raise RuntimeError("The Neuron trainer only supports distributed training through --device xla.")
    return Runtime(
...
class NKIGQAFlashAttentionFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx: Any,
        query: torch.Tensor,
        ...
        from neuronxcc.nki.kernels import flash_attn_bwd, flash_fwd  # noqa: F401
        from neuronxcc.nki.kernels.attention import FlashConfig
        from neuronx_distributed.kernels.kernel_utils import check_xla_bf16_flags, get_seed, move_seed

        del flash_attn_bwd
        if softmax_scale is None:
            softmax_scale = query.shape[-2] ** -0.5
        if seed is None:
            seed = get_seed(dropout_p, query.device)

        batch_size, query_heads, head_dim, seq_len = query.shape
        key_heads = key.shape[1]
        config_kwargs: dict[str, Any] = {"seq_tile_size": int(seq_tile_size)}
        if lse_dtype == "bfloat16" or (
            lse_dtype == "auto"
            and (
                check_xla_bf16_flags()
                or (os.environ.get("XLA_DOWNCAST_BF16", None) == "0" and query.dtype == torch.bfloat16)
            )
        ):
            config_kwargs["lse_dtype"] = "bfloat16"
        config = FlashConfig(**config_kwargs)

        attn_output, lse = flash_fwd[batch_size, key_heads](
            query,
            key,
        ...
        ctx.save_for_backward(query, key, value, attn_output, lse, seed)
        ctx.causal = causal
    ...
    @staticmethod
    def backward(ctx: Any, dout: torch.Tensor, *_args: Any) -> tuple[torch.Tensor | None, ...]:
        from neuronxcc.nki.kernels import flash_attn_bwd

        query, key, value, attn_output, lse, seed = ctx.saved_tensors
        batch_size, _query_heads, _head_dim, _seq_len = query.shape
        key_heads = key.shape[1]
        dout = dout.permute(0, 1, 3, 2)
        attn_output = attn_output.permute(0, 1, 3, 2)
        dq, dk, dv = flash_attn_bwd[batch_size, key_heads](
            query,
            key,
            value,
            attn_output,
...
def nki_gqa_flash_attention(
    ...
    *,
    ...
    if query.device.type != "xla":
        raise RuntimeError("NKI flash attention is only available on XLA/Neuron devices.")
    if query.shape[-2] % int(seq_tile_size) != 0 or key.shape[-2] % int(seq_tile_size) != 0:
        raise NotImplementedError(
            f"nki_flash_1k requires sequence length divisible by {seq_tile_size} "
            f"(query={query.shape[-2]}, key={key.shape[-2]})."
        )
    if query.shape[-1] > 128:
        raise NotImplementedError(f"NKI flash attention supports head_dim <= 128, got {query.shape[-1]}.")
    if query.shape[1] % key.shape[1] != 0:
        raise NotImplementedError(f"query heads ({query.shape[1]}) must divide by kv heads ({key.shape[1]}).")

    from neuronx_distributed.kernels.kernel_utils import cast, permute

    query, key, value = permute(query, key, value)
    query, key, value = cast(query, key, value)
    return NKIGQAFlashAttentionFunc.apply(
        query,
        key,
        value,
        softmax_scale,
...
class NeuronSelfAttention(nn.Module):
    def __init__(self, config: MetisMambaConfig, *, attention_kernel: str, nki_flash_lse_dtype: str) -> None:
        super().__init__()
        self.attention_kernel = attention_kernel
        self.nki_flash_lse_dtype = nki_flash_lse_dtype
    ...
    def _nki_flash_2k(
        ...
        if query.device.type != "xla":
            raise RuntimeError("nki_flash_2k attention is only available on XLA/Neuron devices.")
        if key.shape[1] != query.shape[1]:
            repeats = query.shape[1] // key.shape[1]
            key = repeat_kv(key, repeats)
            value = repeat_kv(value, repeats)
        target_seq = ((seq_len + 2047) // 2048) * 2048
        if target_seq != seq_len:
            pad_len = target_seq - seq_len
    ...
    def _nki_flash_1k(
        ...
        if seq_len != 1024:
            raise NotImplementedError(f"nki_flash_1k is specialized for seq_len=1024, got {seq_len}.")
        return nki_gqa_flash_attention(
            query,
            key,
            value,
            softmax_scale=self.scale,
            seq_tile_size=1024,
            lse_dtype=self.nki_flash_lse_dtype,
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        elif self.attention_kernel == "nki_flash_2k":
            out = self._nki_flash_2k(query, key, value, seq_len)
        elif self.attention_kernel == "nki_flash_1k":
            out = self._nki_flash_1k(query, key, value, seq_len)
    ...
    def __init__(
        ...
        *,
        ...
        if not config.uses_single_latent_moe:
            raise ValueError("Neuron Metis trainer currently supports the single-latent MoE contract.")
        if int(config.moe_num_experts) % int(runtime.world_size) != 0:
            raise ValueError("moe_num_experts must divide runtime world_size for Neuron expert parallel.")
...
class NeuronMetisBlock(nn.Module):
    def __init__(
        self,
        config: MetisMambaConfig,
        *,
        ...
        self.attn = NeuronSelfAttention(
            config,
            attention_kernel=attention_kernel,
            nki_flash_lse_dtype=nki_flash_lse_dtype,
        )
...
class NeuronMetisForCausalLM(nn.Module):
    def __init__(
        self,
        config: MetisMambaConfig,
        *,
    ...
    *,
    ...
    if runtime.rank == 0:
        common_payload = {
            ...
            "checkpoint_format": "neuron_ep_common_and_rank_experts_v1",
            "model_state_dict": (
            ...
            "train_args": vars(args) | {
                ...
                "trainer": "train_metis15_neuron.py",
            },
        }
    ...
    if runtime.world_size > 1:
        ...
        shard_payload = {
            "model_family": config.model_type,
            "checkpoint_format": "neuron_ep_rank_experts_v1",
            "rank": runtime.rank,
            "world_size": runtime.world_size,
...
def load_training_checkpoint(
    *,
    ...
    if checkpoint.get("checkpoint_format") == "neuron_ep_common_and_rank_experts_v1":
        ...
        if runtime.world_size > 1:
            ...
            if not shard_path.exists():
                raise FileNotFoundError(
                    ...
                    "Compact Neuron checkpoints require latest.pt plus latest.rankNNN.pt files."
                )
            shard = torch.load(shard_path, map_location="cpu", weights_only=False)
            if shard.get("checkpoint_format") != "neuron_ep_rank_experts_v1":
    ...
    if load_path != base_path:
        if not load_path.exists():
            raise FileNotFoundError(
                ...
                "For legacy expert-parallel Neuron checkpoints, keep every rank shard next to latest.pt."
            )
...
def _zero_like_device(model: NeuronMetisForCausalLM, shape: tuple[int, ...], *, dtype: torch.dtype = torch.float32) -> torch.Tensor:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Neuron/PJRT static expert-parallel Metis-1.5 trainer.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Materialize optimizer state before step 1 so Neuron compiles the steady-state optimizer graph.",
    )
    ...
    parser.add_argument("--nki-flash-lse-dtype", choices=NKI_FLASH_LSE_DTYPE_CHOICES, default="auto")
...
def main() -> None:
    ...
    try:
        ...
        model = NeuronMetisForCausalLM(
            ...
            nki_flash_lse_dtype=args.nki_flash_lse_dtype,
        ...
        if args.resume_from:
            ...
            if runtime.rank == 0:
                print(
                    f"Resumed Neuron checkpoint from {args.resume_from} "
        ...
        if runtime.rank == 0:
            (out_dir / "train_config.json").write_text(
                json.dumps(
                    ...
                    | {
                        ...
                        "device_kind": runtime.device_kind,
                        "trainer_moe_backend": "neuron_static_ep",
                        "model_config": config.to_dict(),
            ...
            print("Neuron Metis launch", flush=True)
            ...
            print(
                ...
                f"nki_lse={args.nki_flash_lse_dtype} "
                f"moe={args.moe_mode}",
                flush=True,
            )
            ...
            print("  trainer MoE backend: neuron_static_ep", flush=True)
