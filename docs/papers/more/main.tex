...
isolate each axis at matched executed FLOPs, including the control this
...
\item We back it with an axis-isolating ladder at matched executed FLOPs. The
...
(Section~\ref{sec:rm}).
\item A 13-model ablation ladder that isolates each axis at matched executed
...
$q_t^{(r)}$ be the softmax at temperature $\gamma$,
$\hat{k}_t^{(r)} = \sum_j q_{t,j}^{(r)} k_j$ the expected count, and $k_t^{(r)}$
the hard count (Gumbel straight-through during training, $\arg\max$ at
inference). The hard count drives the packed dispatch exactly. The routed
...
\[
  ...
  + \lambda_d \big(\bar{d} - d^\star\big)^2
  + \lambda_z \mathcal{L}_z
  - \lambda_H \mathcal{H}\!\left[p^{(r)}\right],
\]
with $k^\star$, $d^\star$ the target mean width and depth. Expert load balancing
...
\TODO{Report the coefficients and a sensitivity sweep. Two quadratic budget
penalties pulling on a shared compute budget is the most fragile part of this
design; the paper is stronger for saying so than for hoping nobody checks.}

\paragraph{Why one budget and not two.} \TODO{Argue explicitly. If depth and
width are separately budgeted, the model can satisfy both without ever trading
between them, and ``joint allocation'' becomes decoration. The depth--width
correlation analysis in Section~\ref{sec:analysis} is the empirical test of
whether real trading happens, and it is a test we can fail.}
...
\toprule
\# & Model & Recur. & Depth & Width $k$ & Pathway & Mem. & Stored/Active & GF/tok \\
\midrule
1 & Dense, FLOP-matched & -- & 1 & -- & dense & -- & 1.12B / 0.70B & 7.29 \\
2 & Dense, parameter-matched & -- & 1 & -- & dense & -- & 1.82B / 1.41B & 12.96 \\
3 & MoE, $k{=}4$ & -- & 1 & fixed & routed & -- & 1.83B / 0.30B & 4.10 \\
4 & MoE, $k{=}8$ & -- & 1 & fixed & routed & -- & 1.83B / 0.30B & 4.49 \\
5 & Fixed LoopMoE & yes & fixed 2 & fixed & re-routed & -- & 1.83B / 0.30B & 7.30 \\
6 & Loop, pathway frozen & yes & fixed 2 & fixed & frozen & -- & 1.83B / 0.30B & 7.30 \\
7 & MoR + dense FFN & yes & adaptive & -- & dense & -- & 0.72B / 0.30B & 7.28 \\
8 & MoR + fixed-$k$ MoE & yes & adaptive & fixed & re-routed & -- & 1.83B / 0.30B & 7.30 \\
9 & Fixed-depth, adaptive $k$ & yes & fixed 2 & adaptive & re-routed & -- & 1.83B / 0.30B & 7.30 \\
10 & \textbf{MoRE-Core} & yes & adaptive & adaptive & re-routed & -- & 1.83B / 0.30B & 7.30 \\
11 & \textbf{MoRE-RM} & yes & adaptive & adaptive & re-routed & yes & 1.83B / 0.30B & 7.30 \\
\midrule
12 & Random-$k$ control & yes & adaptive & random & re-routed & -- & 1.83B / 0.30B & 7.30 \\
13 & Random-depth control & yes & random & adaptive & re-routed & -- & 1.83B / 0.30B & 7.30 \\
\bottomrule
\end{tabular}
\end{center}

Active parameters are per pass; rows with mean depth 2 execute them roughly
twice. GFLOP/token is \emph{executed} work at the planned policy, including the
pass-level recompute replay, from the same accounting the trainer reports
against. Each row differs from a neighbour in exactly one factor.
...
\item \textbf{Rows 8, 9, 10} isolate depth and width against each other, all at
matched executed FLOPs.
...
recursive model cannot hold 1.5B stored at 0.29B active. That asymmetry is not a
...
Rows 1 and 5--13 are iso-FLOP by construction. Rows 2, 3, 4 are deliberate
...
adds about 0.4 GFLOP/token because expert GEMMs are a minority of the block,
while a second pass adds roughly 3.2. Row~4 is a reference, not a match, and we say so before
someone assumes we tried and failed.

\subsection{Configuration}

\TODO{Proxy manifest table: 10 physical layers (8 Mamba-2 + 2 attention),
$d_{\mathrm{model}}=1792$, latent $896$, expert intermediate $448$, 96 routed + 1
...
\midrule
Depth adaptivity beats a matched fixed depth & rows 5, 8, 10 at iso-FLOP & row 5 $\geq$ row 8 \\
Width adaptivity beats a matched fixed $k$ & rows 8, 9, 10 at iso-FLOP & row 8 $\geq$ row 10 \\
